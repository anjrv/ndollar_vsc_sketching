...
/**
 * Similar to the "Stroke" class
 */
public class Gesture {
    /**
     * List of initial, raw/unchanged Points
     */
    public ArrayList<PointR> RawPoints;
     ...
     */
    public ArrayList<PointR> Points;
    ...
    /**
     * Private constant, having the options: 4, 8
     * It represents the Nth (8th) point in the path to determine the starting vector from the path.
     */
    ...
    /**
     * Constructor
     * @param points List of RAW Points
     */
    public Gesture(ArrayList<PointR> points) {
        this.RawPoints = new ArrayList<PointR>(points); // copy (saved for drawing)
        ...
        // this is now set by a flag in the NDolarRecognizer.cs file with all the other flags
        // rotate so that the centroid-to-1st-point is at zero degrees
        double radians = Utils.AngleInRadians(Utils.Centroid(Points), (PointR)Points.get(0), false);
        ...
        Points = Utils.Scale(Points, NDolarRecognizer.ResampleScale);

        // next, translate to a common origin
        Points = Utils.TranslateCentroidTo(Points, NDolarRecognizer.ResampleOrigin);

        // finally, save the start angle
        // store the start unit vector after post-processing steps
        this.StartUnitVector = Utils.CalcStartUnitVector(Points, StartAngleIndex);
