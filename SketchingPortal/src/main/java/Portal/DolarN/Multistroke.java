...
public class Multistroke {
     ...
     */
    public ArrayList<Gesture> subGestures;  // all possible orderings/directions of this multistroke gesture
     ...
     */
    public Multistroke(String name, ArrayList<ArrayList<PointR>> strokes, boolean createAllPossibleCombinations) {
        this.Name = name;
        ...
        this.OriginalGesture = new Gesture(points);
        ...
        ArrayList<Integer> defaultOrder = new ArrayList<Integer>(strokes.size()); // array of integer indices
        ...
        if (createAllPossibleCombinations == true) {
            ArrayList<ArrayList<Integer>> allOrderings = new ArrayList<ArrayList<Integer>>();
            ...
            this.subGestures = new ArrayList<Gesture>(unistrokes.size());
            for (ArrayList<PointR> entry : unistrokes) {
                Gesture newG = new Gesture(entry);
                this.subGestures.add(newG);
            }
        } else {
            this.subGestures = new ArrayList<Gesture>();
        }
    }
    ...
    public void HeapPermute(int n, ArrayList<Integer> currentOrder, ArrayList<ArrayList<Integer>> allOrders) {
        if (n == 1) {
            ...
            allOrders.add(new ArrayList<Integer>(currentOrder)); // copy
        }
        else {
            for (int i = 0; i < n; i++) {
    ...
    public ArrayList<ArrayList<PointR>> MakeUnistrokes(ArrayList<ArrayList<PointR>> originalStrokes, ArrayList<ArrayList<Integer>> allOrderings) {
        ArrayList<ArrayList<PointR>> allUnistrokes = new ArrayList<ArrayList<PointR>>(); // will contain all possible orderings/direction enumerations of this gesture
        for(ArrayList<Integer> ordering : allOrderings) {
            for (int b = 0; b < Math.pow(2d, ordering.size()); b++) { // decimal value b
                ArrayList<PointR> unistroke = new ArrayList<PointR>(); // we're building a unistroke instead of multistroke now for ease of processing
                for (int i = 0; i < ordering.size(); i++) { // examine b's bits
                    // copy the correct UniStroke
                    ArrayList<PointR> stroke = new ArrayList<PointR>(originalStrokes.get((int)ordering.get(i)));
