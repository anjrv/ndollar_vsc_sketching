...
public class BestListR
{
    /**
     * List of the Results
     */
    private final ArrayList<BestResultR> _nBestList;
     ...
     */
    public ArrayList<BestResultR> getNBestList(){
    ...
    public void AddResult(String name, double score) {
        BestResultR r = new BestResultR(name, score);
        _nBestList.add(r);
    }
        ...
        for(int i = 0 ; i < _nBestList.size() - 1 ; i++){
            for(int j = i + 1 ; j < _nBestList.size() ; j++){
                if(_nBestList.get(i).getScore() < _nBestList.get(j).getScore()){
                    BestResultR aux = new BestResultR( _nBestList.get(i));
    ...
    public String getNameOfTopResult() {
        if (_nBestList.size() > 0) {
            BestResultR r = _nBestList.get(0);
            return r.getName();
        }
        return "";
    }
