...
public class Utils {
     ...
     */
    private static final double MINIMUM_DISTANCE_FOR_EXTREME_POINTS = 45.0;
     ...
     */
    private static final double MIN_DISTANCE_FILTER_1 = 3.0;
     ...
     */
    public static double getAverageWeightedDirection(ArrayList<PointR> subPathOfPoints) {
     ...
     */
    public static ArrayList<Double> getCompleteListOfAverageValuesOfDirections(ArrayList<PointR> pathPoints) {
        ArrayList<Double> arrayListOfAllAngleValues = new ArrayList<>();

        ArrayList<PointR> subPath;
     ...
     */
    public static ArrayList<Double> getCompleteListOfAngles(ArrayList<PointR> pathList) {
        ArrayList<Double> completeListOfValuesOfAngles = new ArrayList<>();

        int n = pathList.size();

        ArrayList<PointR> subPathOfConsecutivePoints;
     ...
     */
    public static double getAverageWeightedAngleOfStreamOfPoints(ArrayList<PointR> pointList) {
        ...
        for (int i = 1; i <= numberOfPoints - 1; i++) {
            P1 = new PointR(pointList.get(i - 1).x, pointList.get(i - 1).y);
            P2 = new PointR(pointList.get(i).x, pointList.get(i).y);
     ...
     */
    public static Pair<ArrayList<PointR>, String> getClockwiseAndCounterClockwiseVariables(ArrayList<PointR> pathPoints) {
        ...
        ArrayList<PointR> arrayListOfCriticalPointsRegardingClockwiseAndCounterClockwise = new ArrayList<>();

        int previousDirection = 0,
                indexOfPreviousDirection = 0;

        int indexOfCriticalPoint;
     ...
     */
    public static double getDirectionInDegreesOfMostPredominantDirection(StrokeR givenStroke) {
        // Direction count
        int[] directionCount = getDirectionCountEnhanced(givenStroke.getPathPoints());

        // the Direction of the Line
        int directionIndex = getPredominantDirectionsVariables(directionCount).getKey().getKey();

        return getAverageDirectionBasedOnMostPredominantDirection(directionIndex, givenStroke.getPathPoints());
    }
     ...
     */
    public static double getDirectionGivenByTwoPointsUpTo360(PointR P1, PointR P2) {
     ...
     * @return the degrees between these points
     */
    public static double getDirectionGivenByTwoPointsUpTo180Reflected(PointR P1, PointR P2) {
     ...
     */
    public static double getAverageDirectionBasedOnMostPredominantDirection(int givenDirectionIndex, ArrayList<PointR> pathPoints) {
     ...
     */
    public static Pair<Double, Pair<ArrayList<Double>, ArrayList<PointR>>> getSharpnessAndAngleVariables(ArrayList<PointR> pathPoints, boolean newValuesAreGiven, double givenDeltaS, double givenEpsilonS) {

        double computedSharpness;

        ArrayList<Double> listOfValuesRepresentingSharpestAngles = new ArrayList<>();
        ArrayList<PointR> listOfPointsRepresentingSharpAngles = new ArrayList<>();
        ...
        for (Double completeListOfValuesOfSharpAngle : completeListOfValuesOfSharpAngles) {
            if (completeListOfValuesOfSharpAngle >= MINIMUM_SHARP_ANGLE_NEW) {
        ...
        if (n >= 3) {
            ...
            double sharpestAngleInTheCluster;
            int clusterStartingIndex,
                    clusterFinishIndex = 0,
                    indexOfSharpestAngleInTheCluster;
                ...
                if (completeListOfBoolValuesRegardingSharpAngles.get(i)) {
                    ...
                    for (int j = i + 1; j < i + 5; j++) {
                        if (j < completeListOfBoolValuesRegardingSharpAngles.size() &&
                                completeListOfBoolValuesRegardingSharpAngles.get(j)) {
        ...
        if (isTheEndPointCloseToTheOverallPath(pathPoints) &&
            ...
            ArrayList<PointR> intersection = new ArrayList<>();
            ...
            double displacementX = (pathPoints.get(pathPoints.size() - 1).x - pathPoints.get(0).x),
                    displacementY = (pathPoints.get(pathPoints.size() - 1).y - pathPoints.get(0).y);

            // index 0 is used to shift the first few points
            for (int i = 1; i <= 4; i++) {
                PointR newPoint = new PointR(pathPoints.get(i).x + displacementX,
                        pathPoints.get(i).y + displacementY);
     ...
     */
    public static double getDistanceBetweenTwoPoints(PointR firstPoint, PointR secondPoint) {
     ...
     */
    public static boolean isTheEndPointCloseToTheOverallPath(ArrayList<PointR> pathList) {
        // the first "25%"
        double beginningPercentage = 0.25;

        PointR lastPoint = pathList.get(pathList.size() - 1);
     ...
     */
    public static int[] getDirectionCountEnhanced(ArrayList<PointR> pathPoints) {
        ...
        for (Double listOfAverageDirectionInDegree : listOfAverageDirectionInDegrees) {
            int myDirection = Direction.getDirectionByDegrees(listOfAverageDirectionInDegree).getIndex();
     ...
     */
    public static boolean overlap(StrokeR stroke1, StrokeR stroke2) {
     ...
     */
    public static boolean isInsideOf(StrokeR stroke1, StrokeR stroke2) {
     ...
     */
    public static ArrayList<StrokeR> removeConsecutiveClosePointsFilter(ArrayList<StrokeR> providedStroke) {
        ArrayList<StrokeR> rez = new ArrayList<>();
        StrokeR resultingStroke = new StrokeR();
        StrokeR copyingStroke = providedStroke.get(0);
    ...
    public static Pair<Double,
            Pair<Pair<Double, Double>,
                    Pair<Double, Double>>> getMinDistanceBetweenExtremitiesOf(PointR s1Start, PointR s1End, PointR s2Start, PointR s2End) {
        double minDist;
     ...
     */
    public static ArrayList<StrokeR> getStrokesAsAMergedStroke(ArrayList<StrokeR> originalStrokeList) {

        ArrayList<StrokeR> mergedStrokeList = new ArrayList<>();
        StrokeR mergedStroke = new StrokeR();
        ...
        if (originalStrokeList.size() > 1) {
            ...
            mergedStroke.beginPath(new PointR(originalStrokeList.get(0).getStartPoint().x, originalStrokeList.get(0).getStartPoint().y));
            for (int i = 1; i < originalStrokeList.get(0).getPathPoints().size(); i++) {
                mergedStroke.addPointToPath(new PointR(originalStrokeList.get(0).getPathPoints().get(i).x, originalStrokeList.get(0).getPathPoints().get(i).y));
            }

            // The starting point of the currently merged strokes. It is not complete yet, so these points might get updated.
            PointR startPointOfMergedPath = new PointR(mergedStroke.getStartPoint().x, mergedStroke.getStartPoint().y);//mergedStroke.getPathPoints().get(0);

            // The end point of the currently merged strokes
            PointR endPointOfMergedPath = new PointR(mergedStroke.getEndPoint().x, mergedStroke.getEndPoint().y);//mergedStroke.getPathPoints().get(mergedStroke.getPathPoints().size() - 1);
            ...
            for (int iteration = 0; iteration < originalStrokeList.size() - 1; iteration++) {
                ...
                for (int i = 0; i < originalStrokeList.size(); i++) {
                    if (!strokesAdded.contains(i)) {
                        double minDistInCurrent = getMinDistanceBetweenExtremitiesOf(startPointOfMergedPath,
                                ...
                                new PointR(originalStrokeList.get(i).getStartPoint().x,
                                        originalStrokeList.get(i).getStartPoint().y),
                                new PointR(originalStrokeList.get(i).getEndPoint().x,
                                        originalStrokeList.get(i).getEndPoint().y)
                        ).getKey();
                ...
                Pair<Double, Pair<Pair<Double, Double>, Pair<Double, Double>>> minimumDistVariable = getMinDistanceBetweenExtremitiesOf(startPointOfMergedPath,
                        endPointOfMergedPath,
                        new PointR(originalStrokeList.get(indexOfNextStroke).getStartPoint().x,
                                originalStrokeList.get(indexOfNextStroke).getStartPoint().y),
                        new PointR(originalStrokeList.get(indexOfNextStroke).getEndPoint().x,
                                originalStrokeList.get(indexOfNextStroke).getEndPoint().y));
                ...
                ArrayList<PointR> pointArrayList = originalStrokeList.get(indexOfNextStroke).getPathPoints();
                ...
                if (minDist == distanceBetweenEndPointOfWhatWeHaveSoFarAndStartPointOfCurrentStroke) {
                    endPointOfMergedPath = new PointR(originalStrokeList.get(indexOfNextStroke).getEndPoint().x, originalStrokeList.get(indexOfNextStroke).getEndPoint().y);
                    // add the current stroke in THIS ORDER at the END of what we have so far
                    displacementX = (mergedStroke.getEndPoint().x - originalStrokeList.get(indexOfNextStroke).getStartPoint().x);
                    displacementY = (mergedStroke.getEndPoint().y - originalStrokeList.get(indexOfNextStroke).getStartPoint().y);

                    for (int point_index = 1; point_index < pointArrayList.size(); point_index++) {
                        mergedStroke.addPointToPath(new PointR(pointArrayList.get(point_index).x + displacementX,
                                pointArrayList.get(point_index).y + displacementY));
                    }

                } else if (minDist == distanceBetweenEndPointOfWhatWeHaveSoFarAndEndPointOfCurrentStroke) {
                    endPointOfMergedPath = new PointR(originalStrokeList.get(indexOfNextStroke).getStartPoint().x, originalStrokeList.get(indexOfNextStroke).getStartPoint().y);
                    // add the current stroke in REVERSE ORDER at the END of what we have so far
                    displacementX = (mergedStroke.getEndPoint().x - originalStrokeList.get(indexOfNextStroke).getEndPoint().x);
                    displacementY = (mergedStroke.getEndPoint().y - originalStrokeList.get(indexOfNextStroke).getEndPoint().y);
                    ...
                    startPointOfMergedPath = new PointR(originalStrokeList.get(indexOfNextStroke).getStartPoint().x,
                            originalStrokeList.get(indexOfNextStroke).getStartPoint().y);
                    // add the current stroke in THIS ORDER at the BEGINNING of what we have so far + update STARTPOINT
                    displacementX = (mergedStroke.getStartPoint().x - originalStrokeList.get(indexOfNextStroke).getEndPoint().x);
                    displacementY = (mergedStroke.getStartPoint().y - originalStrokeList.get(indexOfNextStroke).getEndPoint().y);

                    for (int point_index = pointArrayList.size() - 2; point_index >= 0; point_index--) {
                        mergedStroke.addPointToPathAt(0, new PointR(pointArrayList.get(point_index).x + displacementX,
                                pointArrayList.get(point_index).y + displacementY));
                    }

                } else {
                    startPointOfMergedPath = new PointR(originalStrokeList.get(indexOfNextStroke).getEndPoint().x, originalStrokeList.get(indexOfNextStroke).getEndPoint().y);

                    // start to start
                    // add the current stroke in REVERSE ORDER at the BEGINNING of what we have so far + update STARTPOINT
                    displacementX = (mergedStroke.getStartPoint().x - originalStrokeList.get(indexOfNextStroke).getStartPoint().x);
                    displacementY = (mergedStroke.getStartPoint().y - originalStrokeList.get(indexOfNextStroke).getStartPoint().y);
     ...
     */
    public static Pair<Double, Pair<PointR, PointR>> getArrowPointingVariables(ArrayList<StrokeR> originalGroupOfStrokes) {
        double direction;

        PointR tip = new PointR(0, 0);
        PointR initialPoint = new PointR(0, 0);

        ArrayList<PointR> tail = new ArrayList<>(),
                head = new ArrayList<>();
        ...
        if (originalGroupOfStrokes.size() == 1) {
            ...
            for (int i = 0; i < originalGroupOfStrokes.size(); i++) {
                ...
                if (currentPathLength > longestPathLength) {
                    ...
                    tail = new ArrayList<>(originalGroupOfStrokes.get(i).getPathPoints());
                }
            }

            double shortestDimensionPath = getBiggestDimensionOfStroke(originalGroupOfStrokes.get(indexOfTail));
            int indexOfHead = -1;
            for (int i = 0; i < originalGroupOfStrokes.size(); i++) {
                ...
                if (currentPathLength <= shortestDimensionPath) {
                    ...
                    if (currentPathLength <= shortestDimensionPath * (2.0 / 3)) {
                        head = new ArrayList<>(originalGroupOfStrokes.get(i).getPathPoints());
                    }
                }
            }
            if (head.size() == 0) {
        ...
        if (getDistanceBetweenTwoPoints(tail.get(tail.size() - 1), head.get(0)) >
            ...
            //Tail was reversed
            ArrayList<PointR> tailReversed = reversePathPointsInStroke(tail);
            tail = new ArrayList<>(tailReversed);
        }
        ...
        StrokeR lastPartTailStroke = new StrokeR();
        ...
        PointR tipOnTail = tail.get(tail.size() - 1);
        boolean tipFound = false;
        int indexOfDirectionGivenByTail = Direction.getDirectionByDegrees(direction).getIndex();

        for (PointR pointR : head) {
            double currentDistance = getDistanceBetweenTwoPoints(tipOnTail, pointR);
            double directionBetweenTailAndTip = getDirectionGivenByTwoPointsUpTo360(tipOnTail, pointR);
        ...
        if (!tipFound) {
            ...
            Pair<ArrayList<Double>, ArrayList<PointR>> sharpnessVars = getSharpnessAndAngleVariables(head, false, 0, 0).getValue();
            if (sharpnessVars.getValue().size() == 0) {
                tip.x = (head.get(0).x);
                tip.y = (head.get(0).y);
            } else {
                tip.x = (sharpnessVars.getValue().get(0).x);
                tip.y = (sharpnessVars.getValue().get(0).y);
            }
        }
     ...
     */
    public static double getBiggestDimensionOfStroke(StrokeR s) {
     ...
     */
    public static ArrayList<PointR> reversePathPointsInStroke(ArrayList<PointR> s) {
        ArrayList<PointR> reverseS = new ArrayList<>();
     ...
     */
    public static double getEnclosureScoreHeuristics(ArrayList<StrokeR> originalGroupOfStrokes) {
        ...
        if (originalGroupOfStrokes.size() == 1) {
            ...
            ArrayList<StrokeR> groupOfStrokesMerged = getStrokesAsAMergedStroke(originalGroupOfStrokes);
            ...
            if (numberOfMissingDirections <= NUMBER_OF_MAXIM_MISSING_DIRECTIONS) {
                ...
                Pair<Double, Pair<ArrayList<Double>, ArrayList<PointR>>> sharpnessVariables = getSharpnessAndAngleVariables(groupOfStrokesMerged.get(0).getPathPoints(), false, 0.0, 0.0);
                ...
                ArrayList<PointR> clockwiseFaults = getClockwiseAndCounterClockwiseVariables(groupOfStrokesMerged.get(0).getPathPoints()).getKey();
                ...
                if (enclosureTestsPassed == 3) {
                    enclosureScore = 0.0;
                    enclosureScore += ((1.0 / 3.0) - 0.15 * (1 - Math.max(0.0, Math.min(1.0, ((maxNumberOfSharpAngles - numberOfActualSharpAngles) / (float)maxNumberOfSharpAngles)))));
     ...
     */
    public static Pair<Double, PointR> getLinePointingVariables(ArrayList<StrokeR> originalGroupOfStrokes) {
        StrokeR mergedStroke = getStrokesAsAMergedStroke(originalGroupOfStrokes).get(0);

        double lineDirection = getDirectionInDegreesOfMostPredominantDirection(mergedStroke);

        PointR lineTip = mergedStroke.getEndPoint();
     ...
     */
    public static boolean isText(ArrayList<StrokeR> groupOfStrokes) {
     ...
     */
    public static boolean strokesAreInProximity(StrokeR s1, StrokeR s2, double maxDist) {
        if (maxDist < 0) {
        ...
        verticalProximityCheck = isVerticalProximityCheck(maxDist, stroke1LeftBorder, stroke1RightBorder, stroke2LeftBorder, stroke2RightBorder, verticalProximityCheck);

        // HORIZONTAL proximity:
        // if stroke1 is within stroke2 (horizontally speaking)
        horizontalProximityCheck = isVerticalProximityCheck(maxDist, stroke1TopBorder, stroke1BottomBorder, stroke2TopBorder, stroke2BottomBorder, horizontalProximityCheck);
    ...
    private static boolean isVerticalProximityCheck(double maxDist, double stroke1LeftBorder, double stroke1RightBorder, double stroke2LeftBorder, double stroke2RightBorder, boolean verticalProximityCheck) {
    ...
    public static double getPathLengthR(ArrayList<PointR> points) {
        ...
        for (int i = 1; i < points.size(); i++) {
            length += distanceBetweenPointRs(points.get(i - 1), points.get(i));
        }
        return length;
    }
     ...
     * @return complete length of the path
     */
    public static double getPathLength(ArrayList<PointR> points) {
        double length = 0;
        for (int i = 1; i < points.size(); i++) {
    ...
    public static ArrayList<PointR> RotateByRadians(ArrayList<PointR> points, double radians) {
        ArrayList<PointR> newPoints = new ArrayList<>(points.size());
    ...
    public static PointR CalcStartUnitVector(ArrayList<PointR> points, int index) {
        ...
        PointR v = new PointR(points.get(index).x - points.get(0).x,
                points.get(index).y - points.get(0).y);
    ...
    public static ArrayList<PointR> TranslateCentroidTo(ArrayList<PointR> points, PointR toPt) {
        ...
        for (PointR p : points) {
            newPoints.add(new PointR(p.x + (toPt.x - centroid.x), p.y + (toPt.y - centroid.y)));
        }
        return newPoints;
    }
    ...
    public static ArrayList<PointR> Scale(ArrayList<PointR> pts, SizeR size) {
        ...
        ArrayList<PointR> scaledPts = new ArrayList<>();
        for (PointR p : pts) {
            scaledPts.add(new PointR(p.x * factor, p.y * factor));
        }
        return scaledPts;
    }
    ...
    public static ArrayList<PointR> Resample(ArrayList<PointR> points, int n) {
        ...
        double D = 0.0;
        ArrayList<PointR> srcPts = new ArrayList<>(points);
        ArrayList<PointR> dstPts = new ArrayList<>(n);
    ...
    public static boolean Is1DGesture(ArrayList<PointR> rawPts) {
        ...
        ArrayList<PointR> pts = new ArrayList<>(rawPts);

        // rotate points to 0 (temporarily!)
        double radians = AngleInRadians(Centroid(pts), pts.get(0), false);
        ...
        if ((r.getWidth() == 0) || (r.getHeight() == 0)) {
            ...
            return (r.getWidth() / r.getHeight()) < Recognizer._1DThreshold;
        } else { // else height is shorter side
            return (r.getHeight() / r.getWidth()) < Recognizer._1DThreshold;
        }
    }
     ...
     */
    public static boolean strokesHaveExtremitiesClose(StrokeR stroke1, StrokeR stroke2) {
        double start_to_start,
                start_to_end,
                end_to_start,
                end_to_end;

        start_to_start = distanceBetweenPointRs(new PointR(stroke2.getStartPoint().x, stroke2.getStartPoint().y),
                new PointR(stroke1.getStartPoint().x, stroke1.getStartPoint().y));
        start_to_end = distanceBetweenPointRs(new PointR(stroke2.getEndPoint().x, stroke2.getEndPoint().y),
                new PointR(stroke1.getStartPoint().x, stroke1.getStartPoint().y));
        end_to_start = distanceBetweenPointRs(new PointR(stroke2.getStartPoint().x, stroke2.getStartPoint().y),
                new PointR(stroke1.getEndPoint().x, stroke1.getEndPoint().y));
        end_to_end = distanceBetweenPointRs(new PointR(stroke2.getEndPoint().x, stroke2.getEndPoint().y),
                new PointR(stroke1.getEndPoint().x, stroke1.getEndPoint().y));
     ...
     */
    public static double getTypeMostLimitOfGroup(ArrayList<StrokeR> groupOfStrokes, String type) {
        double result = 0.0;
        switch (type) {
            case "Left":
                result = groupOfStrokes.get(0).getLeftBorderX();
                for (StrokeR groupOfStroke : groupOfStrokes) {
            ...
            case "Right":
                ...
                for (StrokeR ofStroke : groupOfStrokes) {
     ...
     */
    public double getCurvednessOfPath(ArrayList<PointR> pathPoints, boolean newValuesAreGiven, double givenDeltaC, double givenEpsilonC) {
        double curvedness = 0;
